...
class MediaLibrary {
	...
	public function __construct() {
        ...
        if ( $this->doing_alt_text() || get_option( 'a11ytoolkit_admin_bar', true ) ) {
    ...
    private function doing_alt_text() {
        return (bool) get_option( 'a11ytoolkit_media_library_alt_text', true );
    } // End doing_alt_text()
    ...
    private function doing_other_cols() {
        return (bool) get_option( 'a11ytoolkit_media_library_other_cols', true );
    ...
    public function add_column( $columns ) {
        if ( $this->doing_alt_text() ) {
            $columns[ 'alt_text' ] = __( 'Alt Text', 'accessibility-toolkit' );
        }

        if ( $this->doing_other_cols() ) {
            $columns[ 'dimensions' ] = __( 'Dimensions', 'accessibility-toolkit' );
            $columns[ 'type' ]       = __( 'Type', 'accessibility-toolkit' );
            $columns[ 'file_size' ]  = __( 'File Size', 'accessibility-toolkit' );
        }
    ...
    public function render_column( $column_name, $post_id ) {
        if ( $column_name === 'alt_text' && $this->doing_alt_text() ) {
            ...
            echo '<div class="row-actions visible-on-hover"><a href="#" class="alt-text-edit" data-id="' . esc_attr( $post_id ) . '">' . esc_html__( 'Edit', 'accessibility-toolkit' ) . '</a></div>';
        }
    ...
    public function add_file_size_sorting( $clauses ) {
        ...
        $clauses[ 'join' ] .= "
            ...
            ON ({$wpdb->posts}.ID = filesize_meta.post_id AND filesize_meta.meta_key = '_a11ytoolkit_cached_file_size')";
    ...
    public function cache_file_size( $post_id ) {
        $path = get_attached_file( $post_id );
        if ( file_exists( $path ) ) {
            update_post_meta( $post_id, '_a11ytoolkit_cached_file_size', filesize( $path ) );
        }
    } // End cache_file_size()
    ...
    public function enqueue_scripts( $hook ) {
        ...
        wp_enqueue_script( $handle, A11YTOOLKIT_JS_PATH . 'media-library.js', [ 'jquery' ], A11YTOOLKIT_SCRIPT_VERSION, true );
