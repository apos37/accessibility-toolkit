...
class Common {
	...
	public function check_php_version() {
		if ( version_compare( PHP_VERSION, A11YTOOLKIT_MIN_PHP_VERSION, '<=' ) ) {
			add_action( 'admin_init', function() {
				deactivate_plugins( A11YTOOLKIT_BASENAME );
			} );
			add_action( 'admin_notices', function() {
				/* translators: 1: Plugin name, 2: Required PHP version */
				$notice = sprintf( __( '%1$s requires PHP %2$s or newer.', 'accessibility-toolkit' ),
					A11YTOOLKIT_NAME,
					A11YTOOLKIT_MIN_PHP_VERSION
				);
				echo wp_kses_post(
    ...
    public function plugin_row_meta( $links, $file ) {
        $text_domain = A11YTOOLKIT_TEXTDOMAIN;
        if ( $text_domain . '/' . $text_domain . '.php' == $file ) {

            $guide_url = A11YTOOLKIT_GUIDE_URL;
            $docs_url = A11YTOOLKIT_DOCS_URL;
            $support_url = A11YTOOLKIT_SUPPORT_URL;
            $plugin_name = A11YTOOLKIT_NAME;

            $our_links = [
                'guide' => [
                    // translators: Link label for the plugin's user-facing guide.
                    'label' => __( 'How-To Guide', 'accessibility-toolkit' ),
                    'url'   => $guide_url
                ],
                'docs' => [
                    // translators: Link label for the plugin's developer documentation.
                    'label' => __( 'Developer Docs', 'accessibility-toolkit' ),
                    'url'   => $docs_url
                ],
                'support' => [
                    // translators: Link label for the plugin's support page.
                    'label' => __( 'Support', 'accessibility-toolkit' ),
                    'url'   => $support_url
                ],
            ];
            ...
            foreach ( $our_links as $key => $link ) {
                ...
                $aria_label = sprintf( __( '%1$s for %2$s', 'accessibility-toolkit' ), $link[ 'label' ], $plugin_name );
